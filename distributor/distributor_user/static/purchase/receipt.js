...
function get_total(){
    ...
    for (var a = document.querySelectorAll('table.details tbody tr'), i = 0; a[i]; ++i) {
        ...
        var pur_rate=$(cells[6]).val()
        var mrp=$(cells[8]).val()
        var vat_total=0
        discount_type=$(a[i]).find('td:nth-child(12) :selected').data('id');
        discount_val=$(cells[9]).val();
        discount_type_2=$(a[i]).find('td:nth-child(14) :selected').data('id');
        discount_val_2=$(cells[10]).val();
        ...
        $(a[i]).find('td:nth-child(16) ').html(this_total.toFixed(2))
