...
def individual_attendance(request):
	...
	if request.method == 'POST':
		...
		if not is_present:
			new_attendance.leave_type=leave_type
	...
	try:
		...
		pass
	return render(request, 'hr/individual_attendance.html')
...
def apply_leave(request):
	...
	if request.method == 'POST':
		...
		try:
			...
			events_hol=events.filter(attendance_type=2)
			if events_hol:
				return HttpResponse("Selected Date is a holiday")
		except:
			events=[]
		hol=[]
		hol=holiday_calculator(today, today, events, hol)
		...
		if hol:
			return HttpResponse("Today is holiday")
		attendance_given=teacher_attendance.get(date=date,teacher=staff)
		if attendance_given:
...
def attendance_approval	(request):
	...
	attendances_original=teacher_attendance.for_tenant(this_tenant).filter(is_authorized=False).\
							order_by('date').select_related('Teacher')
	if request.method == 'POST':
		attendances=json.loads(request.POST.get('attendances'))
		with transaction.atomic():
			try:
				for data in attendances:
					...
					attendance.is_authorized=is_authorized
					attendance.save()
			except:
				transaction.rollback()
		jsondata = json.dumps(response_data)
