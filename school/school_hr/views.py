...
def registerStudent(request):
	...
	students=Student.objects.for_tenant(this_tenant).filter(user=None, isactive=True).order_by('key')[:500]
	if request.method == 'POST':
...
def registerStaff(request, called_staff_type):
	print(called_staff_type)
	staffs=Teacher.objects.for_tenant(request.user.tenant).filter(staff_type=called_staff_type,user=None)
	print(staffs)
	if request.method == 'POST':
		calltype = request.POST.get('calltype')
		response_data = {}
		...
		#saving the class
		if (calltype == 'mail'):
			staffid=request.POST.get('profileid')
			email=staffs.get(id=staffid).email_id
			response_data['email'] = email
			...
			with transaction.atomic():
				try:
					...
					user=User()
					user.username=username
					if (called_staff_type== 'Teacher'):
						user.user_type='Teacher'
					elif (called_staff_type == 'Admin'):
	...
	return render(request,'hr/new_profile.html', {'items':staffs,'called_for':called_staff_type})
...
def link_leave(request):
	...
	if request.method == 'POST':
		...
		with transaction.atomic():
			try:
				create_leave_cadre_link(request)				
			except:
				transaction.rollback()
		jsondata = json.dumps(response_data)
		return HttpResponse(jsondata)
...
def link_staff_teachers(request):
	...
	if request.method == 'POST':
		...
		with transaction.atomic():
			try:
				create_staff_cadre_link(request)
			except:
				transaction.rollback()
		jsondata = json.dumps(response_data)
		return HttpResponse(jsondata)
		...
		with transaction.atomic():
			try:
				for data in attendances:
					...
					if (absenttype):
						if (absenttype != "Dont"):
							leave_type_selected=leave_type.objects.for_tenant(this_tenant).get(id=absenttype)
							new_attendance.leave_type=leave_type_selected
							staff_leave_selected=staff_leave.objects.for_tenant(this_tenant).\
												get(teacher=teacher, leave_type=leave_type_selected)
							if (staff_leave_selected.numbers > 0):
								staff_leave_selected.numbers-=1
								staff_leave_selected.leaves_taken+=1
							else:
								transaction.rollback()
...
def individual_attendance(request):
	...
	if request.method == 'POST':
		...
		if not is_present:
			leave_type_selected=leave_type.objects.for_tenant(this_tenant).get(id=current_leave_type)
			new_attendance.leave_type=leave_type_selected
			new_attendance.remarks=remarks
			staff_leave_selected=staff_leave.objects.for_tenant(this_tenant).get(teacher=staff, leave_type=leave_type_selected)
			if (staff_leave_selected.numbers > 0):
				staff_leave_selected.numbers-=1
				staff_leave_selected.leaves_taken+=1
			else:
	...
	leave_list=staff_leave.objects.for_tenant(this_tenant).filter(teacher=staff, year=current_year.year).select_related('leave_type')
	leaves=[]
	for item in leave_list:
		#Change it to get data directly from teacher leave list
		# leave=leave_type.objects.for_tenant(this_tenant).get(staffLeave_leaveType=item)
		# leaves_applied=teacher_attendance.objects.filter(teacher=staff, date__range=(start,end), leave_type=leave).count()
		# leaves_available=item.numbers
		# leaves.append({'id':item.leave_type.id,'name':item.leave_type.name, 'applied':leaves_applied,'available':item.numbers})
		if (item.numbers>0):
			leaves.append({'id':item.leave_type.id,'name':item.leave_type.name, 'available':item.numbers})
	return render(request, 'hr/individual_attendance.html',{'leaves':leaves,'extension':extension})
		...
		with transaction.atomic():
			try:
				for data in attendances:
					...
					is_authorized=data['authorize']
					is_rejected=data['reject']
					if (is_authorized):
