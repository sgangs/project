...
def teacher_validate(row, this_tenant, counter):
    data="te"
    tenant=this_tenant
    today=dt.date.today()
    today_string=today.strftime('%y%m%d')
    next_teacher_number='{0:03d}'.format(counter)
    last_teacher=Teacher.objects.filter(tenant=this_tenant).\
            filter(key__contains=today_string).order_by('key').last()
    if last_teacher:
        last_teacher_number=int(last_teacher.key[8:])
        next_teacher_number='{0:03d}'.format(last_teacher_number + counter)
    key=data+str(today_string)+str(next_teacher_number)
    ...
    if (row[0] == None or row[0] == "" or row[1] == None or row[1] == "") :
        ...
        return "Error"
    if (row[4] != "M" and row[4] != "F" and row[4] != "O"):
        transaction.rollback()
        return "Error"
        # return HttpResponse("There is error in uploaded excel")
    if (row [2] != None and row [2] != "" and row [3] != None and row [3] != ""):
        if (type(row [2]) != dt.date or type(row [3]) != dt.date):
            transaction.rollback()
            return "Error"
            # return HttpResponse("There is error in uploaded excel")            
    return row
